...
from replay.metrics import HitRate, NDCG, Experiment, Novelty, Recall, Precision, Coverage, Mean, MAP, MRR, RocAuc, \
    Surprisal, CategoricalDiversity
...
metrics = Experiment(
    [NDCG(K), HitRate(K), Recall(K), Precision(K), Coverage(K)],
    test,
    query_column="user_id",
    item_column="item_id",
...
# model_list = [ALSWrap, ItemKNN, PopRec, RandomRec, SLIM, Word2VecRec]
model_list = [PopRec, ItemKNN]
for elem in model_list:
    ...
    print(
        ...
        Novelty(K, query_column="user_id", item_column="item_id", rating_column="rating")(recs, test_dataset.interactions),
        Precision(K, query_column="user_id", item_column="item_id", rating_column="rating")(recs, test_dataset.interactions),
        Recall(K, query_column="user_id", item_column="item_id", rating_column="rating")(recs, test_dataset.interactions),
        RocAuc(K, query_column="user_id", item_column="item_id", rating_column="rating")(recs, test_dataset.interactions),
        Surprisal(K, query_column="user_id", item_column="item_id", rating_column="rating")(recs, test_dataset.interactions),
      )
    # try:
    #     model.fit(train_dataset)
    #
    ...
    #     recs = encoder.query_and_item_id_encoder.inverse_transform(encoded_recs)
    #
    #     metrics.add_result(str(model), recs)
    # except:
    #     print(str(model))
